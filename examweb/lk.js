...
function clickOnEye(event) {                                    
    ...
    option2.value = "Лёгкие закуски и горячие напитки во время экскурсии (+1000 рублей за каждого посетителя)";
...
function clickOnPen(event) {                           
    ...
    switchLabel1.textContent = "Скидка для пенсионеров";
    let routeOptionF = event.target.parentNode.parentNode.getAttribute("data-option1");
    ...
    switchLabel2.textContent = "Лёгкие закуски и горячие напитки во время экскурсии";
    let routeOptionS = event.target.parentNode.parentNode.getAttribute("data-option2");
